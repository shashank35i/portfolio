...
- Software Engineer Intern, 9X IT Solutions, Hyderabad, Oct 2025-Mar 2026: migrated Spring Boot REST APIs from session-based authentication to stateless JWT and reconfigured Spring Security for token validation on protected requests; centralized user, moderator and admin RBAC in Spring Security authorization rules, eliminating duplicated controller-level access checks; optimized Admin Dashboard queries with DTO projections to fetch only response-required fields, reducing API response payload size by 23%; added pagination to the admin user API to cap records returned per request; analyzed MySQL EXPLAIN plans and benchmarked updated APIs with JMeter, reducing average response time by about 25% and increasing throughput by about 20%. Do not claim indexing, join optimization, undocumented database schema changes or other 9X work not listed here.
